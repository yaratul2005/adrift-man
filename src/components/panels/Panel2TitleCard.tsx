...
export function Panel2TitleCard({ isActive, speedRatio = 0.3 }: { isActive: boolean, speedRatio?: number }) {
  ...
  return (
    <BasePanel isActive={isActive} className="bg-[#0a0e14]">
      <div className="relative z-10 w-full h-full flex flex-col items-center justify-center text-center px-4 overflow-hidden">

        {/* Continuous slow horizontal drift while waiting */}
        <div className={cn(
          "flex flex-col items-center transition-all duration-[8000ms] ease-linear",
          isActive ? "translate-x-4 scale-105" : "translate-x-0 scale-100"
        )}>
          {/* Author */}
          <p className={cn(
