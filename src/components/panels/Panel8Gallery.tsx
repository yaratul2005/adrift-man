...
export function Panel8Gallery({ isActive, speedRatio = 0.3 }: { isActive: boolean, speedRatio?: number }) {
  return (
    <BasePanel isActive={isActive} className="bg-black flex flex-col justify-center items-center">
      ...
      {/* Top Text Overlay */}
      <div className={cn(
        "relative z-20 text-center mb-12 transition-all duration-1000 delay-300",
        ...
        >
          {galleryImages.map((src, idx) => (
            <div
              key={idx}
              className={cn(
                "relative h-full w-[40vw] md:w-[25vw] aspect-[4/3] flex-shrink-0 transition-all duration-1000",
      ...
      {/* Bottom CTA */}
      <div className={cn(
        "relative z-20 mt-8 transition-all duration-1000 delay-700",
        isActive ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
      )}>
        <Link
          href="/gallery"
